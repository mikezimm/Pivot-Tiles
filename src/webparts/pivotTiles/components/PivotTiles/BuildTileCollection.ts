...
export function buildTileCollectionFromWebs(response, pivotProps: IPivotTilesProps , custCategories: ICustomCategories, fixedURL, currentHero){
  ...
  //          https://stackoverflow.com/questions/47755247/typescript-array-map-return-object
    ...
    for (let item of response) {
      ...
      //Do all to modified
        item.modified = (getColumnValue(webProps,item,'colModified'));
        //2020-11-16: Not required for web ==>>  item.modifiedByID = (getColumnValue(pivotProps,item,'colModifiedById')); // This is required for addPersonVariations
        //2020-11-16: Not required for web ==>>  item.modifiedByTitle = (getColumnValue(pivotProps,item,'colModifiedByTitle')); // This is required for addPersonVariations

        //2020-11-16: Not required for web ==>>  item = addPersonVariations(item,'modifiedBy');

        //2020-11-16: Not required for web ==>>  if(modifiedByTitles.indexOf(item.modifiedByTitle) === -1) { modifiedByTitles.push(item.modifiedByTitle); }
        //2020-11-16: Not required for web ==>>  if(modifiedByIDs.indexOf(item.modifiedByID) === -1) { modifiedByIDs.push(item.modifiedByID); }
        ...
        //2020-11-16: Not required for web ==>>  modifiedByInfo.cats = pushPersonsToCategories(modifiedByInfo.cats, item.modifiedBy);
        ...
        //2020-11-16: Not required for web ==>>  item.modifiedInitials = getNameInitials( item.modifiedByTitle );
        item.modifiedNote = item.modifiedSimpleDate;

      //Do all to created
        item.created = (getColumnValue(webProps,item,'colCreated'));
        //2020-11-16: Not required for web ==>>  item.createdByID = (getColumnValue(pivotProps,item,'colCreatedById')); // This is required for addPersonVariations
        //2020-11-16: Not required for web ==>>  item.createdByTitle = (getColumnValue(pivotProps,item,'colCreatedByTitle')); // This is required for addPersonVariations

        //2020-11-16: Not required for web ==>>  item = addPersonVariations(item,'createdBy');

        //2020-11-16: Not required for web ==>>  if(createdByTitles.indexOf(item.createdByTitle) === -1) { createdByTitles.push(item.createdByTitle); }
        //2020-11-16: Not required for web ==>>  if(createdByIDs.indexOf(item.createdByID) === -1) { createdByIDs.push(item.createdByID); }
        ...
        //2020-11-16: Not required for web ==>>  createdByInfo.cats = pushPersonsToCategories(createdByInfo.cats, item.createdBy);
    ...
    for (let item of response) {
      ...
      //2020-11-16: Not required for web ==>>  item.createdBy.cats.bestFormat[0] = createdByInfo.bestFormat;
      //2020-11-16: Not required for web ==>>  item.modifiedBy.cats.bestFormat[0] = modifiedByInfo.bestFormat;
    }
   ...
   */
    ...
    if ( custCategories.type !== 'tileCategory' ) {
      if ( custCategories.column && custCategories.column.length > 0 ) {
        if ( custCategories.column.indexOf( webProps.colTitleText ) === -1 ) { custSearchTitle = false; }
        if ( custCategories.column.indexOf( webProps.colHoverText ) === -1 ) { custSearchDesc = false; }
        if ( custCategories.column.indexOf( webProps.colGoToLink ) === -1 ) { custSearchHref = false; }
        if ( custCategories.column.indexOf( webProps.colCategory ) === -1 ) { custSearchCate = false; }
        //2020-11-16: Not required for web ==>>  if ( custCategories.column.indexOf( 'ModifiedBy/Title' ) === -1 ) { custSearchModBy = false; }
        //2020-11-16: Not required for web ==>>  if ( custCategories.column.indexOf( 'CreatedBy/Title' ) === -1 ) { custSearchCreateBy = false; }
      ...
      if ( custCategories.type === 'tileCategory' ) {
        ...
        custCatLogi.map( custCat => {
          ...
          if (  custSearchTitle && title.search(regex) > -1 ) {
            ...
            match = true;
          //2020-11-16: Not required for web ==>>  } else if (  custSearchModBy && modifiedByTitle.search(regex) > -1 ) {
          //2020-11-16: Not required for web ==>>    match = true;
          //2020-11-16: Not required for web ==>>  } else if (  custSearchCreateBy && createdByTitle.search(regex) > -1 ) {
          //2020-11-16: Not required for web ==>>    match = true;
          }
